pub mod model {
    ...
    impl Model {
        ...
        unsafe fn load_mesh(&mut self, ind_mesh: usize) {
            let pos_acc_ind = self.json["meshes"][ind_mesh]["primitives"][0]["attributes"]
                ["POSITION"]
                .as_u64()
                .expect("Expected a u64 value in JSON") as usize;
            let normal_acc_ind = self.json["meshes"][ind_mesh]["primitives"][0]["attributes"]
                ["NORMAL"]
                .as_u64()
                .expect("Expected a u64 value in JSON") as usize;

            let tex_acc_ind = self.json["meshes"][ind_mesh]["primitives"][0]["attributes"]
                ["TEXCOORD_0"]
                .as_u64()
                .expect("Expected a u64 value in JSON") as usize;
            ...
            let normal_vec =
                Model::get_floats(self, self.json["accessors"][normal_acc_ind].clone());
